 ...
 * @author deepak
 */
public class categoryviewforcustomer extends HttpServlet {
...
//
//         out.println("<!--content-->");
            out.println("<h3>Category Details</h3>");
            out.println("<form name=\"thisform\" action=\"categoryviewforcustomer\">");
            out.println("Select Category:<br>");
            out.println("<select name=\"categoryid\" size=1>");
            categoryDAO rc = new categoryDAO();
            ArrayList<categoryBean> ar = rc.findAll();
            for (categoryBean r : ar) {
                out.println("<option name=" + r.getCategoryid() + " value=" + r.getCategoryid() + ">");
                out.println(r.getCategoryname());
                out.println("</option>");
            }
            out.println("</select>");
            out.println("<input type=\"submit\" value=\"Show\">");
            out.println("</form>");
            String rcat = request.getParameter("categoryid");
            if (rcat != null) {
                int rcategoryid = Integer.parseInt(rcat);
                out.println("<table>");
                out.println("<tr><th>Product Name</th><th>Product Price</th> <th colspan=\"2\"></th></tr>");
                productDAO rd = new productDAO();
                ArrayList<productBean> list = rd.findProductsForCategory(rcategoryid);
                for (productBean bean : list) {
                    out.println("<tr>");
                    out.println("<td>" + bean.getProductname() + "</td>");
                    out.println("<td>" + bean.getProductprice() + "</td>");
                    out.println("<td><a href=\"customerAddCart?method=add&productid=" + bean.getProductid() + "&productname="+bean.getProductname()+"&productprice="+bean.getProductprice()+"&opt=0\">Add to card</a></td>");
//out.println("<td><a href=\"masterorderList?method=add&productid=&itemname=${prod.modelno}&price=${prod.price}&opt=0\">Add to cart</a></td>");
